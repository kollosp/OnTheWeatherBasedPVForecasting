...
def test(forecasting_horizon, prediction_step_len, models, models_names, instance=0, excluded_dims=None, n=None, n_steps=None, included_dims=None):
...
def main(forecasting_horizons, prediction_step_len, models, n_values, n_steps_values, instance_values, covered_dimensions=None, excluded_dimensions=None, dims=None):
    """
    ...
    covered_dimensions:
        a set of dimensions that should be either included or excluded in model dimension list. Function creates a powerset
    ...
    experiment_configs = [
        ...
        for fh, n, n_steps, instance, ed in product(forecasting_horizons, n_values, n_steps_values, instance_values, dims)
    ]

    first = True
    for i,config in enumerate(experiment_configs):
        print(f"Running experiment: fh={config['fh']} n={config['n']}, n_steps={config['n_steps']}, instance={config['instance']}, dims={config['excluded_dims']}")
        # continue
        try:
            prediction_df, metrics_df = test(
                ...
                included_dims=config["excluded_dims"],
            )
